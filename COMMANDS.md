```
/ping - Testing

/profile list - List all profiles
/profile create name:"" - Create a profile
/profile remove name:"" - Delete a profile (Requires verification via reacting with a random emoji)
/profile edit name:"" promptId:"" - Edit system prompt of a profile
/profile edit name:"" model:"" - Edit model of a profile
/profile clear name:"" - Clear message history (Requries verification)
/profile swap name:"" - Swap to a profile

/prompt create id:"" prompt:"" - Create a prompt template
/prompt remove promptId:"" - Remove a prompt template (Requires verification)
/prompt list - List all prompt templates
/prompt default prompt:"" - Set default prompt

/setup - Wizzard
```